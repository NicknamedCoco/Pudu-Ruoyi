  ...
  <div>
    ...
    <div class="tagcloud-all"
      ...
      <!--<div class="addTag">
        ...
        <el-button  class="button-new-tag" size="small" @click="addTag">增加评价</el-button>
      </div>-->
      <a v-for="item in tagList" :style="'color:' + item.color + ';top: 0;left: 0;filter:none;'">{{item.name}}</a>
    </div>
  </div>
</template>

<script>
  import {getTags,addTag} from '@/api/front'
export default {
  name: "Tags",
  data() {
    return {

<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="registrationList" @selection-change="handleSelectionChange">
      <el-table-column label="病历号" align="center" prop="number" width="110"/>
      ...
      <el-table-column label="预约就诊时间" align="center" prop="createTime" width="180">
        <template slot-scope="scope">
      ...
      <el-table-column label="是否采纳" align="center" prop="accept" >
        <template slot-scope="scope">
          ...
          <el-tag v-if="scope.row.accept === 0" type="info">未处理</el-tag>
        </template>
      </el-table-column>

      <el-table-column label="满意度" align="center" prop="evaluate" >
        <template slot-scope="scope">
          <el-tag v-if="scope.row.evaluate === '' || scope.row.evaluate === null" type="info">未评价</el-tag>
      ...
      >
        <template slot-scope="scope">
          <el-button v-if="scope.row.accept !== 0 && scope.row.evaluate === null"
                     size="mini"
                     type="text"
                     icon="el-icon-view"
                     @click="handleEvaluate(scope.row)"
          >评价</el-button>
          <el-button v-if="scope.row.accept === 0 && scope.row.status === 1"
                     size="mini"
                     type="text"
                     icon="el-icon-view"
...
<script>
import {accept, evaluate, listRegistration, notAccept, openInvoice} from "@/api/system/booking";
